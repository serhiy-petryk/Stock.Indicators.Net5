...
public static partial class Indicator
{
        ...
        where TQuote : IQuote
    {
        ...
        List<IchimokuResult> results = new List<IchimokuResult>(length);
        ...
        for (int i = 0; i < length; i++)
        {
            ...
            IchimokuResult r = new IchimokuResult(q.Date);
            results.Add(r);
        ...
        where TQuote : IQuote
    {
        if (i >= tenkanPeriods - 1)
        {
            ...
            result.TenkanSen = (min == decimal.MaxValue) ? (decimal?)null : (min + max) / 2;
        }
    }
        ...
        where TQuote : IQuote
    {
        if (i >= kijunPeriods - 1)
        {
            ...
            result.KijunSen = (min == decimal.MaxValue) ? (decimal?)null : (min + max) / 2;
        }
    }
        ...
        where TQuote : IQuote
    {
        if (i >= senkouOffset + senkouBPeriods - 1)
        {
            ...
            result.SenkouSpanB = (min == decimal.MaxValue) ? (decimal?)null : (min + max) / 2;
        }
    }
