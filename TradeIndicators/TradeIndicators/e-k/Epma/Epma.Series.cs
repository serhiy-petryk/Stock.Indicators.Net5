...
public static partial class Indicator
{
        ...
        int lookbackPeriods)
    {
        ...
        // initialize
        List<SlopeResult> slopeResults = tpList.CalcSlope(lookbackPeriods).ToList();

        int length = slopeResults.Count;
        List<EpmaResult> results = new List<EpmaResult>(length);
        ...
        for (int i = 0; i < length; i++)
        {
            ...
            EpmaResult r = new EpmaResult(s.Date)
            {
