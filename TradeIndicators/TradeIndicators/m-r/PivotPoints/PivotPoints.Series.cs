...
public static partial class Indicator
{
        ...
        where TQuote : IQuote
    {
        ...
        List<PivotPointsResult> results = new List<PivotPointsResult>(length);
        ...
        where TPivotPoint : IPivotPoint, new()
    {
        switch (pointType)
        {
            case PivotPointType.Standard: return GetPivotPointStandard<TPivotPoint>(high, low, close);
            case PivotPointType.Camarilla: return GetPivotPointCamarilla<TPivotPoint>(high, low, close);
            case PivotPointType.Demark: return GetPivotPointDemark<TPivotPoint>(open, high, low, close);
            case PivotPointType.Fibonacci: return GetPivotPointFibonacci<TPivotPoint>(high, low, close);
            case PivotPointType.Woodie: return GetPivotPointWoodie<TPivotPoint>(open, high, low);
            default: throw new ArgumentOutOfRangeException("pointType", pointType, "Invalid pointType provided.");
        }
    }
    ...
    private static int GetWindowNumber(DateTime d, PeriodSize windowSize)
    {
        switch (windowSize)
        {
            case PeriodSize.Month: return d.Month;
            case PeriodSize.Week: return EnglishCalendar.GetWeekOfYear(d, EnglishCalendarWeekRule, EnglishFirstDayOfWeek);
            case PeriodSize.Day: return d.Day;
            case PeriodSize.OneHour: return d.Hour;
            default: throw new ArgumentOutOfRangeException("windowSize", windowSize, string.Format(EnglishCulture, "Pivot Points does not support PeriodSize of {0}.  See documentation for valid options.", Enum.GetName(typeof(PeriodSize), windowSize)));
        }
    }
}
}
